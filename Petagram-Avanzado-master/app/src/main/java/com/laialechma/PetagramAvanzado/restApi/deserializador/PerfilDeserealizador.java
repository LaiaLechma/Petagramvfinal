...
public class PerfilDeserealizador implements JsonDeserializer <PerfilResponse> {
    ...
    private ArrayList<Contacto> deserealizadorPerfil(JsonArray jsonArray) {
        ...
        for(int i = 0; i < jsonArray.size(); i++ ){
            JsonObject usersDataObject = jsonArray.get(i).getAsJsonObject();
            Contacto item = new Contacto();
            JsonObject imageJson = usersDataObject.getAsJsonObject(JsonKeys.TIMELINE_IMAGES);
            ...
            JsonObject likesJson = usersDataObject.getAsJsonObject(JsonKeys.TIMELINE_LIKES);
